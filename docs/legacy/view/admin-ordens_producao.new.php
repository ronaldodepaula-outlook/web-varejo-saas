...
<head>
    ...
    <title>Ordens de Produção - SAS Multi</title>
...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Quando o usuário selecionar um produto no modal de consumo, preencher o custo unitário automaticamente
            const consumoSelect = document.getElementById('consumoProdutoId');
            if (consumoSelect) {
                consumoSelect.addEventListener('change', function() {
                    const opt = this.options[this.selectedIndex];
                    const preco = opt?.dataset?.precoCusto || '';
                    if (preco !== '') {
                        const inputCusto = document.getElementById('consumoCustoUnitario');
                        if (inputCusto) inputCusto.value = preco;
                    }
                });
            }
        ...
        async function carregarProdutosEstoque() {
            try {
                ...
                if (response.ok) {
                    ...
                    // API retorna uma estrutura paginada: { success:true, data: { current_page, data: [ ... ] } }
                    // Suportar também formatos alternativos por segurança.
                    const produtos = (json && json.data && Array.isArray(json.data.data)) ? json.data.data
                    ...
                    console.error('Resposta não ok ao buscar produtos:', response.status, texto);
                    mostrarNotificacao('Erro ao carregar lista de produtos: ' + response.status, 'error');
                }
            } catch (error) {
        ...
        function preencherSelectProdutos(produtos) {
            ...
            // Ordenar por descrição para melhor usabilidade
            ...
            produtos.forEach(produto => {
                ...
                const codigo = produto.codigo_barras ? ` [${produto.codigo_barras}]` : '';
                const unidade = produto.unidade_medida ? ` (${produto.unidade_medida})` : '';
                const preco = produto.preco_custo ? ` - ${formatarMoeda(produto.preco_custo)}` : '';

                option.textContent = `${produto.descricao}${codigo}${unidade}${preco}`;

                // Adicionar atributos data- para uso posterior (ex: preencher custo automaticamente)
                option.dataset.codigoBarras = produto.codigo_barras || '';
                option.dataset.unidadeMedida = produto.unidade_medida || '';
                option.dataset.precoCusto = produto.preco_custo || '';

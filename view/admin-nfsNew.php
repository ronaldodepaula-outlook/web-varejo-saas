...
<head>
    ...
    <style>
        ...
        /* When sidebar is hidden (collapsed) move it out of view */
        ...
        /* Smaller devices: sidebar starts collapsed by default */
        ...
        /* NexusFlow-style sidebar sections */
        .sidebar-header {
            padding-bottom: 12px;
        ...
        .logo-icon { font-size: 1.4rem; color: var(--primary-color); }
        .logo-text { font-weight: 700; color: var(--secondary-color); }

        .sidebar-nav { padding: 6px 4px; }
        .nav-section { margin-top: 10px; }
        .nav-header { padding: 8px 10px; color: #6c757d; font-size: 0.78rem; font-weight: 600; }
        .nav-item { margin: 4px 0; }
        .nav-item .nav-link { display: flex; align-items: center; gap: 10px; padding: 8px 10px; color: #495057; border-radius: 8px; text-decoration: none; }
        .nav-item .nav-link:hover { background: rgba(0,0,0,0.03); color: var(--primary-color); }
        .nav-link .nav-icon { width: 22px; text-align: center; }
        .nav-link .nav-text { flex: 1; }
        .nav-link.active { background: rgba(52,152,219,0.08); color: var(--primary-color); }
    </style>
</head>
<body>
    <!-- Sidebar -->
    <nav class="sidebar" id="appSidebar" aria-hidden="true">
        ...
        <div class="sidebar-nav">
            ...
            <div class="nav-section" data-role="admin">
                ...
                <div class="nav-item">
                    <a href="admin-empresas.php" class="nav-link">
    ...
    <!-- Overlay used on mobile to close the sidebar -->
    <div id="sidebarOverlay" class="sidebar-overlay d-none" aria-hidden="true"></div>
    ...
    <script>
        ...
        async function processarImportacao() {
            ...
            for (let i = 0; i < filesToUpload.length; i++) {
                ...
                try {
                    const token = '<?php echo $token; ?>';
                    const response = await fetch(API_CONFIG.getImportarUrl(), {
                        method: 'POST',
                        headers: {
                            'Authorization': `Bearer ${token}`,
        ...
        async function carregarFiliais() {
            // Simulação - em uma implementação real, você buscaria as filiais da empresa
            const select = document.getElementById('id_filial');
            select.innerHTML = '<option value="">Selecione a filial</option>';
            
            // Exemplo de filiais - substitua por chamada à API real
            const filiais = [
                { id: 1, nome: 'Matriz' },
                { id: 2, nome: 'Filial 1' },
                { id: 3, nome: 'Filial 2' }
    ...
    <script>
        ...
        (function(){
            ...
            // Initialize state: collapsed on small screens, else read from sessionStorage
            const saved = (function(){ try { return sessionStorage.getItem('sidebarCollapsed'); } catch(e){ return null; }})();
            if (saved !== null) {
            ...
            // Ensure overlay visibility updates on resize
            window.addEventListener('resize', function(){
                if (window.innerWidth >= 992) {
                    overlay.classList.add('d-none');
                    // keep sidebar visible by default on desktop unless explicitly collapsed
                } else {
